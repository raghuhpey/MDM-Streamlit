...
def fetch_classification_run_input_df(run_id: str) -> pd.DataFrame:
 ...
 keep_cols = []
 for c in ["run_id", "row_hash"] + input_cols:
     if c in run_df.columns and c not in keep_cols:
         keep_cols.append(c)
 return run_df[keep_cols].copy().reset_index(drop=True)
...
def fetch_classification_runs_summary_df() -> pd.DataFrame:
 ...
 info = _get_sql_dataset_location()
 predicted_case = " OR ".join([f"{_quote_identifier(c)} IS NOT NULL" for c in CLASSIFICATION_OUTPUT_COLUMNS])
 query = f"""
 SELECT
     run_id,
     MIN(file_name) AS file_name,
     MIN(uploaded_by) AS uploaded_by,
     MIN(uploaded_ts) AS uploaded_ts,
     MIN(run_status) AS run_status,
     MIN(worker_scenario_id) AS worker_scenario_id,
     MIN(backend_run_id) AS backend_run_id,
     MIN(backend_trigger_id) AS backend_trigger_id,
     MIN(prediction_started_ts) AS prediction_started_ts,
     MIN(prediction_completed_ts) AS prediction_completed_ts,
     COUNT(*) AS row_count,
     SUM(CASE WHEN ({predicted_case}) THEN 1 ELSE 0 END) AS predicted_row_count,
     MAX(updated_ts) AS updated_ts
 FROM {info['quoted_table_name']}
 GROUP BY run_id
 ORDER BY MAX(updated_ts) DESC
 """
 summary_df = _query_to_df(query)
 if summary_df.empty:
     return pd.DataFrame(columns=base_columns)
 summary_df["predicted_row_count"] = pd.to_numeric(summary_df["predicted_row_count"], errors="coerce").fillna(0).astype(int)
 summary_df["row_count"] = pd.to_numeric(summary_df["row_count"], errors="coerce").fillna(0).astype(int)
 summary_df["remaining_row_count"] = (summary_df["row_count"] - summary_df["predicted_row_count"]).clip(lower=0).astype(int)
 ...
 return summary_df[base_columns]
...
def request_stop_run_in_classification_table(run_id: str):
 ...
 if current_scenario_run is None:
     raise ValueError(
         "The backend scenario has not started running yet. "
         "Please wait a few seconds and try Stop again."
     )
 stored_backend_run_id = str(meta.get("backend_run_id") or "").strip()
 
 if stored_backend_run_id and current_scenario_run.id != stored_backend_run_id:
     raise ValueError(
         f"Another scenario run is currently active "
         f"(current={current_scenario_run.id}, expected={stored_backend_run_id}). "
         "Refusing to abort the wrong run."
     )
 scenario.abort()
 mark_run_stopped(run_id)
...
def persist_prediction_results_to_classification_table(run_id: str, result_df: pd.DataFrame):
 ...
 rename_map = {
     ...
     "llm_as_a_judge_op": "llm_judge_op",
 }
 ...
 # Best path: use original row_hash carried through prediction
 if "row_hash" in result_work_df.columns:
     aligned_result_df = (
 ...
 # Fallback: align by row order only if row counts match
 ...
 # Last fallback: recompute row_hash from overlapping original input cols
 else:
 ...
 print("========== PERSIST DEBUG ==========")
 print("run_id:", run_id)
 print("run_df rows:", len(run_df))
 print("result_work_df rows:", len(result_work_df))
 print("aligned_result_df rows:", len(aligned_result_df))
 for col in available_output_cols:
     print(f"{col} non-null count:", int(aligned_result_df[col].notna().sum()))
 print("row_hash non-null count:", int(aligned_result_df["row_hash"].notna().sum()))
 print("===================================")
 info = _get_sql_dataset_location()
