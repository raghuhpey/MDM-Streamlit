...
folder_path = "/data/dss_install/managed_folders/MDM_PARTS_CLASSIFICATION/sdCGzd1A"



client_aoai = AzureOpenAI(
    api_key="sk-WSYwqK94KaLosJStD88Mdg",
    azure_endpoint="https://dev-emea-cloudapi.alstom.com/10222/N4/V1/",
    api_version="2024-02-01"
)

client_gpt = AzureOpenAI(
    api_key="sk-l9moESKkdVd1g2IAHsbAGg",
    azure_endpoint="https://dev-emea-cloudapi.alstom.com/10222/N4/V1/",
    api_version="2024-02-01"
)
...
#     'embedding_cache': f'combined_data_embedding.parquet',
}
...
def get_highest_embedding_number(data_folder):
    ...
    for file_path in existing_files:
        ...
        if folder_path in file_path:
            match = pattern.search(file_path)  # ← use search instead of match
            if match:
...
def gov_index_vector():
    folder = dataiku.Folder("FAISS_KB")
    filename = "governance_faiss.index"
...
#     gov_index = faiss.read_index(os.path.join(folder_path, "governance_faiss.index"))
#     faiss.write_index(gov_index, os.path.join(folder_path, "governance_faiss.index"))
...
def gov_metadata_vector():    
    # Read JSON
    with folder.get_download_stream("governance_metadata.json") as f:
        gov_metadata = json.load(f)

    # Write JSON
    with folder.get_writer("governance_metadata.json") as w:
        w.write(json.dumps(gov_metadata).encode("utf-8"))

    return gov_metadata
# def gov_metadata_vector():
#     with open(os.path.join(folder_path, "governance_metadata.json"), "r") as f:
#         gov_metadata = json.load(f)
    
#     with open(os.path.join(folder_path, "governance_metadata.json"), "w") as f:
...
def extract_unspsc_candidates(retrieved_items):
    ...
    for item in retrieved_items:
        ...
        family = item["Family Name English"]

        if code not in candidates:
            candidates[code] = {
                "family": family,
                "parent": item["Parent information"],
                "description": item["Description of Code"]
            }
    return candidates
...
def build_rag_prompt(part_description, retrieved_items, unspsc_candidates, model_preds=None):
    ...
    for item in retrieved_items:
        ...
        line = (
            f"- Family: {item['Family Name English']}, "
            f"Parent: {item['Parent information']}, "
            f"Description: {item['Description of Code']}, "
            f"UNSPSC: {item['UNSPSC - UNSPSC']}, "
    ...
    candidate_context = "\n".join(
        [
            ...
            f"Parent {info['parent']}, Description {info['description']}"
...
Look at all the candidtaes in the candidate_context and governance_context and choose the most appropriate UNSPSC code.
...
Review the information entered and check the closest Family and Parent match for the entered Part description.
Cross-reference the entries in the UNSPSC column.
Only map UNSPSC codes from the given data and not from any external source outisde the Alstom master data used to create the knowledge base.
