...
<div class="empty-state-box">
 ...
 for _, run in runs_df.iterrows():
     ...
     with row_cols[4]:
         st.markdown(f'<div class="runs-cell">{int(run.get("predicted_row_count", 0))}</div>', unsafe_allow_html=True)
     with row_cols[5]:
         st.markdown(f'<div class="runs-cell">{int(run.get("remaining_row_count", 0))}</div>', unsafe_allow_html=True)
     with row_cols[6]:
         st.markdown(
